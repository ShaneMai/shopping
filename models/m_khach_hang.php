...
class M_khach_hang extends database {
    ...
    function themKhachHang($ten_khach_hang,$gioi_tinh,$ngay_sinh,$dia_chi,$dien_thoai,$email,$ghi_chu) {
        $query = "INSERT INTO khach_hang(ten_khach_hang,gioi_tinh,ngay_sinh,dia_chi,dien_thoai,email,ghi_chu) ";
        $query.= "VALUES(?,?,?,?,?,?,?)";
        $this->setQuery($query);
        function themKhachHang($ten_khach_hang,$gioi_tinh,$ngay_sinh,$dia_chi,$dien_thoai,$email,$ghi_chu) {
            $query = "INSERT INTO khach_hang(ten_khach_hang,gioi_tinh,ngay_sinh,dia_chi,dien_thoai,email,ghi_chu) ";
            $query.= "VALUES(?,?,?,?,?,?,?)";
            $this->setQuery($query);
            $result = $this->execute(array($ten_khach_hang,$gioi_tinh,$ngay_sinh,$dia_chi,$dien_thoai,$email,$ghi_chu));
            if($result)
